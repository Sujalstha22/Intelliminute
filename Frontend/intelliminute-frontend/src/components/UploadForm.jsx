...
const UploadForm = ({ setProcessing, onRequireAuth }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const meetingId =
        meetingData?.id || meetingData?._id || res?.data?.meetingId;

      if (!meetingId) {
        throw new Error("Missing meeting ID from backend");
      }

      const finalData = {
        id: meetingId,

        // backend meeting fields
        ...meetingData,

        // flatten analysis into root
        summary: analysis.summary || [],
        action_items: analysis.action_items || [],
        decision_items: analysis.decisions || [],
        key_points: analysis.key_points || [],
        participants: analysis.participants || [],

        dominant_speaker: res.data.dominant_speaker,
        speaker_timeline: res.data.speaker_timeline,
  ...
  return (
    <div className="w-full font-dm-sans flex flex-col gap-3">
      {!isAuthenticated && (
        <div className="anim-slide-in bg-yellow-50 border border-yellow-100 text-yellow-700 text-[12.5px] px-4 py-3 rounded-xl">
          You are not logged in. Please sign in to upload and generate meeting
          minutes.
        </div>
      )}
      <style>{`
      ...
      {/* File-level error (format / size / network — NOT auth) */}
      {error && (
      ...
      {/* Generate button — shown when file is picked */}
      {status === "ready" && (
        <button
